...
template <unsigned char sda_pin, unsigned char scl_pin, unsigned int bus_speed, unsigned char port_name>
class AS5600T
{
    public:
        int32_t angle; 
    ...
    public:
        ...
        { 
            ...
            while (init_res != 0 && cnt > 0)
            {
                ...
                i2c_write_reg(I2C_ADDRESS, ZPOS_H_ADR, 0); 
                i2c_write_reg(I2C_ADDRESS, ZPOS_L_ADR, 0); 
        ...
        void calibrate(uint16_t zero_value)
        {
            uint8_t raw_h = (zero_value>>8)&0xff;
            uint8_t raw_l = (zero_value>>0)&0xff;
